...
_.extend(AIC, {
    ...
    renderChart: function(hasData) {
        ...
        if (!AIC.firstRendering && AIC.state.queryData.data.length) {
            ...
            if (AIC._isTimeSeries() && AIC.state.queryData.metadata.canRender === false) {
                jQuery("#nothingToDisplayMessage").html(adhocDesigner.getMessage("ADH_1214_ICHARTS_NO_TIME_SERIES_DATA"));
                ...
                var containerWidth = jQuery('#chartContainer').width();
                var containerHeight = jQuery('#chartContainer').height();
    ...
    resize: function() {
        if (AIC.state.chartState.chartType == "pie") {
            AIC.render();
        }
    },
...
_.extend(AIC, {
    mediator: {
        ...
        eventToAction: {
            ...
            "chart:formatOptionsChanged": function() {
                // Update client state.
                var updateIfChanged = function(stateChanged, optionName) {
                    var chartFormatDialog = AIC.ui.controls.chartFormatDialog;

                    if (AIC.state.chartState[optionName] != chartFormatDialog.getOption(optionName)) {
                        AIC.state.chartState[optionName] = chartFormatDialog.getOption(optionName);
                ...
                stateChanged = updateIfChanged(stateChanged, "xAxisRotation");
                stateChanged = updateIfChanged(stateChanged, "yAxisRotation");
                stateChanged = updateIfChanged(stateChanged, "xAxisStep");
                stateChanged = updateIfChanged(stateChanged, "yAxisStep");
                stateChanged = updateIfChanged(stateChanged, "showMeasureOnValueAxis");
                stateChanged = updateIfChanged(stateChanged, "showDataPoints");
    ...
    ui: {
        controls: {
            ...
            chartFormatDialog: {
                ...
                init: function() {
                    var me = AIC.ui.controls.chartFormatDialog;

                    me._container = jQuery('#chartFormatDialog');

                    me._applyBtn = me._container.find(".saveButton");
                    me._cancelBtn = me._container.find(".cancelButton");

                    me._xAxisRotation = me._container.find("#xAxisRotation");
                    me._yAxisRotation = me._container.find("#yAxisRotation");
                    me._xAxisStep = me._container.find("#xAxisStep");
                    me._yAxisStep = me._container.find("#yAxisStep");
                    me._showMeasureOnValueAxis = me._container.find("#showMeasureOnValueAxis");
                    me._showDataPoints = me._container.find("#showDataPoints");


                    me._applyBtn.on("click", function() {
                        me._extractData();

                        AIC.serviceBus.fireEvent("chart:formatOptionsChanged");
                    });

                    me._cancelBtn.on("click", function() {
                        me.hide();
                    });
                },

                update: function(options) {
                    _.extend(AIC.ui.controls.chartFormatDialog._formatOptions, options);
                },

                getOption: function(option) {
                    return AIC.ui.controls.chartFormatDialog._formatOptions[option];
                },

                render: function() {
                    var me = AIC.ui.controls.chartFormatDialog;

                    me._xAxisRotation.val(me._formatOptions.xAxisRotation);
                    me._yAxisRotation.val(me._formatOptions.yAxisRotation);
                    me._xAxisStep.val(me._formatOptions.xAxisStep);
                    me._yAxisStep.val(me._formatOptions.yAxisStep);
                    me._showMeasureOnValueAxis.prop("checked", me._formatOptions.showMeasureOnValueAxis);
                    me._showDataPoints.prop("checked", me._formatOptions.showDataPoints);
                },
                ...
                _extractData: function() {
                    ...
                    var xAxisRotation = me._xAxisRotation.val();
                    var yAxisRotation = me._yAxisRotation.val();
                    var xAxisStep = me._xAxisStep.val();
                    var yAxisStep = me._yAxisStep.val();
                    var showMeasureOnValueAxis = me._showMeasureOnValueAxis.prop("checked");
                    var showDataPoints = me._showDataPoints.prop("checked");

                    if (ValidationModule.validateEntry({element: me._xAxisRotation[0], method: "isInteger"}, false) &&
                        ValidationModule.validateEntry({element: me._xAxisRotation[0], method: "minMax",
                                options: {min: -90, max: 90}}, false)) {
                        me._formatOptions.xAxisRotation = parseInt(xAxisRotation);
                    } else {
                        me._xAxisRotation.val(me._formatOptions.xAxisRotation);
                    }

                    if (ValidationModule.validateEntry({element: me._yAxisRotation[0], method: "isInteger"}, false) &&
                        ValidationModule.validateEntry({element: me._yAxisRotation[0], method: "minMax",
                                options: {min: -90, max: 90}}, false)) {
                        me._formatOptions.yAxisRotation = parseInt(yAxisRotation);
                    } else {
                        me._yAxisRotation.val(me._formatOptions.yAxisRotation);
                    }

                    if (ValidationModule.validateEntry({element: me._xAxisStep[0], method: "isInteger"}, false) &&
                        ValidationModule.validateEntry({element: me._xAxisStep[0], method: "minMax",
                            options: {min: 1, max: 2147483647}}, false)) {
                        me._formatOptions.xAxisStep = parseInt(xAxisStep);
                    } else {
                        me._xAxisStep.val(me._formatOptions.xAxisStep);
                    }

                    if (ValidationModule.validateEntry({element: me._yAxisStep[0], method: "isInteger"}, false) &&
                        ValidationModule.validateEntry({element: me._yAxisStep[0], method: "minMax",
                            options: {min: 1, max: 2147483647}}, false)) {
                        me._formatOptions.yAxisStep = parseInt(yAxisStep);
                    } else {
                        me._yAxisStep.val(me._formatOptions.yAxisStep);
                    }

                    me._formatOptions.showMeasureOnValueAxis = showMeasureOnValueAxis;
                    me._formatOptions.showDataPoints = showDataPoints;
                }
            }
        },
        ...
        update: function(state) {
            ...
            AIC.ui.controls.chartFormatDialog.update({
                xAxisRotation: state.chartState.xAxisRotation,
                yAxisRotation: state.chartState.yAxisRotation,
                xAxisStep: state.chartState.xAxisStep,
                yAxisStep: state.chartState.yAxisStep,
                showMeasureOnValueAxis: state.chartState.showMeasureOnValueAxis,
                showDataPoints: state.chartState.showDataPoints
            });
        },
        ...
        render: function() {
            ...
            AIC.ui.controls.chartFormatDialog.render();
        ...
        if(!AIC.initd) {
            ...
            jQuery('#chartMenu').on('click','li',function(){
                if (jQuery(this).is('[action="chart-types"]')) {
                    ...
                    dialogs.popup.show(AIC.ui.dialogs.formatDialog[0]);
            ...
            AIC.ui.dialogs = {
                ...
                formatDialog: jQuery('#chartFormatDialog')
            };
            ...
            AIC.ui.controls.chartFormatDialog.init();
            AIC.ui.controls.dataLevelSelector.init();
        }
        AIC.ui.dialogs.typeSelector.hide();
        AIC.ui.dialogs.groupSelector.hide();
        AIC.ui.dialogs.formatDialog.hide();
...
_.extend(AIC,{
     ...
     */
    updateCalcFieldAndView: function(fieldName, formulaId, argList){
        var callback = function(state){
        ...
        designerBase.sendRequest("co_updateFieldAndView", ["fieldName=" + encodeText(fieldName),
            "formula=" + formulaId,
            "args=" + argList],
            callback, null);
    }
...
_.extend(AIC,{
    ...
    showAddHierarchyConfirm: function(hierarchyName, dimension, onOk) {
        if (AIC.fromSiblingHierarchy(hierarchyName, dimension)) {
            adhocDesigner.addConfirmDialog.show({ ok : function() {
                if (AIC.isFiltersApplied(dimension)) {
                    dialogs.systemConfirm.show(adhocDesigner.getMessage("ADH_CROSSTAB_LAST_FILTERED_LEVEL"), 5000);
                    return;
                }
                onOk();
    ...
    isFiltersApplied: function(dimension, axis) {
        var levelNames = _.pluck(AIC.getLevelObjectsFromDimension(dimension, axis), "levelUniqueName");
        var filterNames = _.pluck(AIC.state.existingFilters, "filterLabel");
...
_.extend(AIC,{
    ...
    appendDimensionWithLevel: function(level, axis){
        ...
        meta.axis = axis;
        if (AIC.showAddHierarchyConfirm(meta.hierarchyName, meta.dimension, function() {
...
_.extend(AIC,{
    ...
    fromSiblingHierarchy : function(hierarchy, dimension, axis) {
        if (!hierarchy) {
            return false;
        }
        var levelsInAxis = this.getLevelsFromDimension(dimension, axis);
    ...
    getAvailableFieldsNodeBySelection: function (level, dimension, item) {
        ...
        if (selectionCategory.area == designerBase.AVAILABLE_FIELDS_AREA) {
            if (item.hasChilds()) {
                ...
                meta.level = null;
                meta.hierarchyName = (item.param.extra && item.param.extra.isHierarchy) ? item.param.extra.id : undefined;
            } else {
                ...
                meta.hierarchyName = item.param.extra.hierarchyName;
            }
            meta.index = -1;
