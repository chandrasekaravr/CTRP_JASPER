...
AdHocCrosstab.showAddHierarchyConfirm = function(hierarchyName, dimension, onOk) {
    if (localContext.fromSiblingHierarchy(hierarchyName, dimension)) {
        adhocDesigner.addConfirmDialog.show({ ok : function() {
            if (AdHocCrosstab.isFiltersApplied(dimension)) {
...
AdHocCrosstab.isFiltersApplied = function(dimension, axis) {
    var levelNames = _.pluck(localContext.state.getLevelObjectsFromDimension(dimension, axis), "levelUniqueName");
    var filterNames = _.pluck(localContext.state.existingFilters, "filterLabel");
