...
var adHocFilterModule = {
    ...
    normalizeDateFilterValue:function (podId) {
    ...
    populateValuesAndSave: function(podId, selectedOperation, filterType) {
        ...
        if (_.str.include(adHocFilterModule.filterType[selectedOperation], 'Select')) {
            var jSelectElement = jQuery('#' + podId + '_filterInput');
            var oldValues = jSelectElement.val();
            jSelectElement.html('');
            // The following code is rewritten with pure javascript in order to support lagre lists of values.
            this.getFieldValues(podId, function() {
                // Set previously selected value - actual for multiselects
                var selectElement = jSelectElement[0];
                ...
                for (var i = 0; i < optionsLength; i++) {
                    ...
                    if (oldValues !== null && oldValues.constructor === Array && _.contains(oldValues, optionValue)) {
                        option.selected = true;
                    } else if (oldValues === optionValue) {
                        option.selected = true;
                    }
                }
    ...
    _createTextInput:function (podId, firstLevel, isDate) {
        ...
        if (!isDate) {
            $(textInput).observe("change", function() {
                adHocFilterModule.onChangeForInput(podId);
            });
        } else {
            ...
            jQuery(textInput).change(function (event) {
                adHocFilterModule.onChangeForDateInputInline($(event.target));
            });
        }
        var label = this._constructLabel();
        ...
        if (filterType === "singleSelect" || filterType === "singleInput") {
            ...
            for (var i = 0; i < optionsLength; i++) {
                ...
                if (option.selected) {
                    ...
                    literalObj.value = this.formatOrEncodeExpr(dataType, option.value);
...
//            formattedExpr = date ? date.toLocaleString() : value;
    ...
    isDataTypeDecimal: function (dataType) {
        return (dataType === this.DOUBLE || dataType === this.LONG);
    },

    isDataTypeInteger: function (dataType) {
        return dataType === this.INT;
    },
    ...
    createDecimalValidator:function (dataType) {
        var that = this;
        return function(value) {
            value = that.formatNumericValue(value,dataType);
            var res = ValidationModule.methods.isDecimal(value);
            return {
                isValid: res === null,
                errorMessage: res
            };
    ...
    checkInputs:function (podId) {
        ...
        if (filterType === "multipleSelect" || filterType === "singleSelect") {
            ...
            //first input
            var firstInputElement = $(podId + "_filterInput");
            var firstInputValue = firstInputElement.value;
            if (this.isDataTypeNumeric(dataType)) {
                if (this.isDataTypeDecimal(dataType)) {
                    firstInputValue = this.formatNumericValue(firstInputValue, dataType);
                   valid = ValidationModule.validateEntry({
                        validator: this.createDecimalValidator(dataType).bind(this),
                        element: firstInputElement
                    }, false);
                } else if (this.isDataTypeInteger(dataType)) {
                    valid = ValidationModule.validateEntry({element: firstInputElement, method: "isInteger"}, false);
                }
            } else if (dataType === "Date") {
                valid = this.dateValidator(firstInputValue);
            } else if (dataType === "Timestamp") {
                valid = this.timestampValidator(firstInputValue);
            } else {
            ...
            if (filterType === "multipleInput") {
                if (valid) {
                    var secondInputElement = $(podId + "_filterInput2");
                    var secondInputValue = secondInputElement.value;
                    if (this.isDataTypeNumeric(dataType)) {
                        if (this.isDataTypeDecimal(dataType)) {
                            secondInputValue = this.formatNumericValue(secondInputValue, dataType);
                            valid = ValidationModule.validateEntry({
                                    validator: this.createDecimalValidator(dataType).bind(this),
                                    element: secondInputElement
                                }, false);
                            if (valid) {
                                valid = parseFloat(firstInputValue) < parseFloat(secondInputValue);
                            }
                        } else if (this.isDataTypeInteger(dataType)) {
                            valid = ValidationModule.validateEntry({element: secondInputElement, method: "isInteger"}, false);
                            if (valid) {
                                valid = parseInt(firstInputValue) < parseInt(secondInputValue);
                            }
                        }
                    } else if (dataType === "Date") {
                        return this.dateValidator(secondInputValue);
                    } else if (dataType === "Timestamp") {
                        return this.timestampValidator(secondInputValue);
                    } else {
                        //shouldn't get here
    ...
    render:function (state, container) {
        if (state && !_.isEmpty(state)) {
            ...
            var html = adHocFilterModule.filterPodTemplate(state);

            // The following code is rewritten with pure javascript in order to support lagre lists of values.
            adHocFilterModule.optimizedInsertIntoDom(container, html);
            ...
            // The following code is rewritten with pure javascript in order to support lagre lists of values.
            var isOLAP = adHocFilterModule.isOlapMode();
            if (_.isArray(state.existingFilters)) {
                _.each(state.existingFilters, function(filter) {
                    var filterPrefix = isOLAP ? filter.filterLabel : '';
                    var filterInputId = filterPrefix + 'Pod' + filter.filterId + '_filterInput';

                    var input = document.getElementById(filterInputId);
                    if (input && input.tagName.toLowerCase() === 'select') {
                        var parent = input.parentNode;
                        var className = parent.className;
                        if (className.indexOf('control') != -1 && className.indexOf('select') != -1 && className.indexOf('multiple') != -1) {
                            if (input.options.length > 7 && (!isChrome() || (isChrome() && input.options.length < 1000)) ) {
                                var childNodes = parent.childNodes;
                                var childNodesLength = childNodes.length;
                                for (var i = 0; i < childNodesLength; i++) {
                                    var child = childNodes[i];
                                    if (child.tagName && child.tagName.toLowerCase() === 'div' && child.className.indexOf('sizer') != -1) {
                                        child.className = child.className.replace('hidden', '');
                                        layoutModule.createSizer(parent, {sizer: child});
                                    }
                                }
                            }
                        }
    ...
    //TODO Extract this method as generic util method (taken from IC logic)
    optimizedInsertIntoDom: function(container, content) {

        // Check presence of element in DOM
        var isElementInDom = function(elem) {
