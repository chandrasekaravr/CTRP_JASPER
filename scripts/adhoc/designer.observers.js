...
adhocDesigner.observeCustomEvents = function custom(){
    jQuery('#designer').bind({
    	'layout_update': function(evt,data){
            ...
            if((!data || !data.isAdhocDesignerTriggered) && localContext.getMode().indexOf('ichart') >= 0 && AIC.chart) {
                var chartContainer = jQuery('#chartContainer');

                AIC.chart.setSize(chartContainer.width(), chartContainer.height());
            }
        },
...
adhocDesigner.observeDisplayManagerEvents = function() {
    ...
    jQuery('#' + adhocDesigner.DISPLAY_MANAGER_ID).bind('lm:addItem', function(event, options) {
        ...
        localContext.lmHandlersMap[options.axis].addItem(options.dimension, options.index, options.level, options.levelIndex, options.isMeasure, options.uri, options.hierarchyName);
    });
